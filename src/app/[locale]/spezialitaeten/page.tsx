        ...
        <div className="container px-6">
          <Tabs defaultValue="olio" className="w-full">
            ...
            <div className="flex justify-center mb-6 md:mb-10">
              <TabsList className="h-auto flex-wrap justify-center gap-1.5 md:gap-2 bg-transparent p-0">
                {categories.map((cat) => (
                  <TabsTrigger 
                    key={cat} 
                    value={cat}
                    className="data-[state=active]:bg-primary data-[state=active]:text-white data-[state=inactive]:bg-secondary data-[state=inactive]:text-foreground border-0 px-3 py-1.5 md:px-6 md:py-2.5 text-xs md:text-sm font-medium transition-all duration-300 hover:bg-secondary/80"
                  >
            ...
            {categories.map((cat) => (
              ...
              >
                ...
                <div className="relative h-40 md:h-72 overflow-hidden mb-6 md:mb-10">
                  <Image 
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <h2 className="font-display text-2xl md:text-5xl text-white">
                      {t(`categories.${cat}` as any)}
                    </h2>
                  </div>
                </div>
                ...
                {/* Products Grid */}
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-10">
                  {products[cat].map((product) => (
                    ...
                    >
                      ...
                      <div className="aspect-[4/3] relative img-zoom bg-secondary/30">
                        ...
                        {product.price && (
                          <div className="absolute top-2 right-2 md:top-4 md:right-4 bg-accent text-white px-2 py-1 md:px-3 md:py-1.5 text-[10px] md:text-sm font-medium">
                            {product.price}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="p-3 md:p-7 flex flex-col flex-1">
                        <h3 className="font-display text-sm md:text-xl text-primary mb-2 md:mb-3 leading-tight">
                          {product.name}
                        </h3>
                        
                        <span className="inline-block text-[10px] md:text-xs font-medium px-2 py-0.5 md:px-2.5 md:py-1 bg-secondary text-primary/70 mb-2 md:mb-4 self-start">
                          {product.origin}
                        </span>
                        
                        <p className="text-muted-foreground text-xs md:text-sm leading-relaxed mb-3 md:mb-6 flex-1 hidden sm:block">
                        ...
                        <a
                          ...
                          className="inline-flex items-center gap-1 md:gap-2 text-xs md:text-sm font-medium text-accent hover:text-primary transition-colors mt-auto"
                        >
